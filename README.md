...
- [ ] Выбран вариант 2.
- [ ] Указаны провайдер, ОС, vCPU, RAM, диск и IP с скрытым последним октетом.
- [ ] Приложение открывается по адресу VPS.
- [ ] В `/health` возвращается `ok`.
- [ ] Настроен вебхук и автоматический деплой.
- [ ] Тестовый коммит автоматически обновил приложение.
- [ ] Добавлены скриншот Cursor с Remote SSH и работающего сервиса в Coolify.
- [ ] Подготовлено короткое описание сложностей.
- [ ] Ссылка на приватный репозиторий добавлена в текст сдачи, но доступ открыт только преподавателю.
